...
`bundle.py` packs `src/cc_interproject/` into the single-file installer the README tells readers
...
user's machine and it imports nothing from `cc_interproject` at runtime — only at build time, to
