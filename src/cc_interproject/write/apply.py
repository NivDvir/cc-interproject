...
from cc_interproject.run_state import Action, Run
from cc_interproject.write import blocks
from cc_interproject.write.plan import MARKER_VERSION
...
def execute(run: Run, on_progress: Callable[[str, int, int], None]) -> Path:
    ...
    try:
        ...
        print(f"cc-interproject: install error: {exc}", file=sys.stderr)  # noqa: T201
