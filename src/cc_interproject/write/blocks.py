"""The `<!-- cc-interproject:begin/end -->` marker convention used to make a piece of installed text
...
BEGIN = "<!-- cc-interproject:begin id={id} v={v} -->"
END = "<!-- cc-interproject:end id={id} -->"
...
def contains(text: str, marker_id: str) -> bool:
    ...
    pattern = re.compile(r"<!-- cc-interproject:begin id=" + re.escape(marker_id) + r" v=\d+ -->")
...
def strip(text: str, marker_id: str) -> tuple[str, bool]:
    ...
    """
    pattern = re.compile(
        r"<!-- cc-interproject:begin id="
        + re.escape(marker_id)
        + r" v=\d+ -->\r?\n"
        + r".*?"
        + r"<!-- cc-interproject:end id="
        + re.escape(marker_id)
        + r" -->",
        re.DOTALL,
    )
