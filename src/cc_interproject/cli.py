...
from cc_interproject import VERSION
from cc_interproject.heads import preflight
from cc_interproject.manifest import status, uninstall
from cc_interproject.run_state import Run, new_run
from cc_interproject.wizard import launch, terminal
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="cc-interproject",
        description="Claude Code Inter-Project Communication: install the inter-project setup.",
    )
    parser.add_argument("--version", action="version", version=f"cc-interproject {VERSION}")
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print("\ncc-interproject: aborted", file=sys.stderr)
        return EXIT_ABORTED
...
def _preflight(run: Run) -> int | None:
    ...
    if preflight.find_claude(run) is None:
        print("cc-interproject: the claude CLI was not found on PATH", file=sys.stderr)
    ...
    if not preflight.auth_ping(run):
        print("cc-interproject: claude is not logged in (run `claude /login`)", file=sys.stderr)
...
def _status_line(message: str) -> None:
    print(f"cc-interproject: {message}", file=sys.stderr)
