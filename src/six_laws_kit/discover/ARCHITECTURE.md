...
| File | Job |
|---|---|
| `walk.py` | `os.scandir` walk from a root directory. Finds every `CLAUDE.md`, applies the per-OS skip list (`paths.skip_names()`) at every level it enters, caps recursion depth and total directories scanned, reports progress every 200 directories, and never follows a symlink or reparse point. |
